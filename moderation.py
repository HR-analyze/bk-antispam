...
def moderation_variants(text: str) -> str:
    """Добавляет латиницу/символы, которыми обходят фильтр: '$'/'s' -> 'с'."""
...
def direct_gender_job_fallback(text: str) -> bool:
    ...
    who = r"(?:девочк\w*|девуш\w*|женщин\w*|парн\w*|мужчин\w*|мальчик\w*|ребят\w*)"
    return bool(re.search(rf"\b(?:нужн(?:ы|а|о|ы)|ищу|ищем|требуется|требуются|возьму)\s+{who}\b", normalized))
...
def classify_message(text: str, has_link: bool = False):
    ...
    """
    probe = moderation_variants(text)
    if has_link or contains_link(text):
        return "link"
    if direct_gender_job_fallback(probe) or direct_child_job_fallback(probe):
        return "job_spam"
    reason = classify(probe)
    if reason is None and spaced_job_fallback(probe):
        return "job_spam"
    return reason
