...
NEGATIVE_PHRASES = {
    "обслуживание ужасное", "обслуживание ужасно", "ужасное обслуживание", "ужасный сервис", "ужасное место", "херня а не место", "говно а не",
    "скам проект", "скам-проект", "кто хочет денег", "легкий досуг", "лёгкий досуг", "легкий секс", "лёгкий секс", "детское порно", "детское порн", "кровь девственницы",
    "child porn", "child pornography", "sexual content involving minors",
}
JOB_PHRASES = {
    "подработка", "подработку", "подработке", "подработки", "подработать", "шабашка", "шабашку", "шабашке", "шабашки", "хорошая подработка",
    "хорошо оплачиваемая подработка", "хорошо оплачиваемую подработку", "оплачиваемая подработка", "оплачиваемую подработку", "ищу кандидатов", "ищем кандидатов", "нужны кандидаты", "ищу сотрудников",
    "нужны сотрудники", "требуются сотрудники", "требуются люди", "нужны люди", "нужны мужчины", "нужны мужчина", "нужны женщины", "нужны женщина", "мужчины и женщины", "мужчины или женщины", "мужчины женщины",
    "парни и девушки", "парни или девушки", "парни девушки", "нужны парни", "нужны девушки", "нужен парень", "нужна девушка", "мужчины на работу", "женщины на работу", "мужчина на работу", "женщина на работу",
    "ребята на работу", "ребята для работы", "молодые люди на работу", "открыла магазин", "открыл магазин", "открыли магазин", "открываем магазин", "открыла точку", "открыл точку", "открыли точку", "новая схема", "новой схемы",
    "есть варианты", "есть вариант", "варианты работы", "вариант работы", "новая работа", "новые вакансии", "новая вакансия", "есть вакансия", "есть вакансии", "срочно нужны", "срочно требуются", "нужны на сегодня", "нужны на завтра",
    "работа на пару часов", "работы на пару часов", "работа на несколько часов", "на несколько часов", "на пару часов", "предоплата", "предоплата имеется", "предоплата есть", "оплата сразу", "выплата сразу", "деньги сразу",
...
JOB_SIGNALS = ("подработ", "шабаш", "кандидат", "сотрудник", "ваканс", "предоплат", "оплата", "выплата", "заработ", "схем", "магазин", "точк", "пару часов", "несколько часов", "мужчин", "женщин", "парни", "девушки", "парень", "девушка", "ребят", "пишите", "в личку", "в лс")
OBFUSCATED_JOB_PATTERNS = (r"\b[wv]абашк\w*\b", r"\bш[аa]б[аa]шк\w*\b", r"\bп[оo]д[рr][аa]б[оo]тк\w*\b", r"\bпод\s*работ\w*\b")
PAID_TASK_SIGNALS = (
    "нужен человек", "нужна помощь", "ищу человека", "ищу кто", "нужен кто", "кто сможет", "кто сможет помочь", "кто сможет присмотреть", "присмотреть за", "присмотрит за", "посидеть с", "посидеть за", "помочь с", "нужно сделать", "нужен на", "нужна на", "ищу на завтра", "ищу на сегодня", "на постоянную основу", "на постоянной основе", "за 3 часа", "за 2 часа", "за час", "в день", "в сутки", "плачу", "оплачу", "оплата", "выплата",
)
...
TASK_REQUEST_PHRASES = {
    "кто сможет помочь", "кто-нибудь сможет помочь", "кто нибудь сможет помочь", "кто может помочь", "кто-нибудь может помочь", "кто нибудь может помочь", "кто может помочь с", "кто-нибудь может помочь с", "кто нибудь может помочь с", "кто сможет помочь с", "кто-нибудь сможет помочь с", "кто нибудь сможет помочь с", "кто-нибудь знает, кто может помочь", "кто нибудь знает кто может помочь", "кто-нибудь знает кто может помочь", "кто нибудь знает, кто может помочь", "кто знает кто может помочь", "кто знает, кто может помочь", "кто знает кто сможет помочь", "кто знает, кто сможет помочь", "есть кто сможет помочь", "есть кто может помочь", "есть кто поможет", "нужен человек помочь", "нужна помощь с", "ищу человека для помощи", "ищу человека помочь", "ищу кто поможет", "нужен кто поможет", "кто поможет с переездом", "кто может помочь с переездом", "кто сможет помочь с переездом", "кто поможет перевезти", "кто может перевезти", "кто сможет перевезти",
}
...
def raw_spaced(text: str) -> str:
    text = unicodedata.normalize("NFKC", text or "").casefold()
    return re.sub(r"\s+", " ", re.sub(r"[^а-яёa-z0-9₽@]+", " ", text)).strip()

def compact_spaced(text: str) -> str:
    return re.sub(r"\s+", "", text)

def compact_raw(text: str) -> str:
    """Remove separators while preserving original digits, so '5 0 0 0' becomes '5000'."""
    return re.sub(r"[^а-яёa-z0-9₽@]+", "", unicodedata.normalize("NFKC", text or "").casefold())
...
def _is_paid_task_spam(normalized: str, spaced: str, raw: str) -> bool:
    ...
    # Critical anti-obfuscation layer: preserve digits and collapse separators.
    # Examples: 'подра ботка 5 0 0 0' -> 'подработка5000', '5 000' -> '5000'.
    compact = compact_raw(raw)
    has_obfuscated_work = any(token in compact for token in ("подработка", "подработку", "подработке", "подработки", "подработать", "шабашка", "шабашку", "вакансия", "вакансии"))
    compact_has_amount = bool(re.search(r"(?:^|[^0-9])\d{2,6}(?:[^0-9]|$)", compact)) or bool(re.search(r"(?:подработка|подработку|подработке|подработки|шабашка|вакансия)\d{2,6}", compact))
    spaced_digit_amount = bool(re.search(r"\b\d(?:\s+\d){1,7}\b", raw))

    if (has_currency_amount or (has_bare_amount and (has_payment_context or has_duration)) or spaced_digit_amount or compact_has_amount) and (task_signal_count >= 1 or has_obfuscated_work):
        return True
