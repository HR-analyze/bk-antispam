...
NEGATIVE_PHRASES = {
    ...
    "лёгкий досуг", "легкий секс", "лёгкий секс", "детское порно", "детское порн",
    "кровь девственницы", "child porn", "child pornography", "sexual content involving minors",
}

JOB_PHRASES = {
    "подработка", "подработку", "подработке", "подработки", "подработать", "шабашка", "шабашку", "шабашке", "шабашки",
    "хорошая подработка", "хорошо оплачиваемая подработка", "хорошо оплачиваемую подработку", "оплачиваемая подработка", "оплачиваемую подработку",
    "ищу кандидатов", "ищем кандидатов", "нужны кандидаты", "ищу сотрудников", "нужны сотрудники", "требуются сотрудники", "требуются люди", "нужны люди",
    "нужны мужчины", "нужны мужчина", "нужны женщины", "нужны женщина", "мужчины и женщины", "мужчины или женщины", "мужчины женщины",
    "парни и девушки", "парни или девушки", "парни девушки", "нужны парни", "нужны девушки", "нужен парень", "нужна девушка",
    "мужчины на работу", "женщины на работу", "мужчина на работу", "женщина на работу", "ребята на работу", "ребята для работы", "молодые люди на работу",
    "открыла магазин", "открыл магазин", "открыли магазин", "открываем магазин", "открыла точку", "открыл точку", "открыли точку", "новая схема", "новой схемы",
    "есть варианты", "есть вариант", "варианты работы", "вариант работы", "новая работа", "новые вакансии", "новая вакансия", "есть вакансия", "есть вакансии",
    "срочно нужны", "срочно требуются", "нужны на сегодня", "нужны на завтра", "работа на пару часов", "работы на пару часов", "работа на несколько часов",
    "на несколько часов", "на пару часов", "предоплата", "предоплата имеется", "предоплата есть", "оплата сразу", "выплата сразу", "деньги сразу",
    "легкий заработок", "лёгкий заработок", "легкие деньги", "лёгкие деньги", "заработок без опыта", "без опыта", "пишите в личку", "пишите в лс", "пишите мне", "кому интересно пишите",
}

JOB_SIGNALS = (
    "подработ", "шабаш", "кандидат", "сотрудник", "ваканс", "предоплат", "оплата", "выплата", "заработ", "схем", "магазин", "точк", "пару часов",
    "несколько часов", "мужчин", "женщин", "парни", "девушки", "парень", "девушка", "ребят", "пишите", "в личку", "в лс",
)

OBFUSCATED_JOB_PATTERNS = (
    r"\b[wv]абашк\w*\b", r"\bш[аa]б[аa]шк\w*\b", r"\bп[оo]д[рr][аa]б[оo]тк\w*\b", r"\bпод\s*работ\w*\b",
)

PAID_TASK_SIGNALS = (
    "нужен человек", "нужна помощь", "ищу человека", "ищу кто", "нужен кто", "кто сможет", "кто сможет помочь", "кто сможет присмотреть",
    "присмотреть за", "присмотрит за", "посидеть с", "посидеть за", "помочь с", "нужно сделать", "нужен на", "нужна на", "ищу на завтра",
...
TASK_REQUEST_PHRASES = {
    ...
    "кто может помочь с", "кто-нибудь может помочь с", "кто нибудь может помочь с", "кто сможет помочь с", "кто-нибудь сможет помочь с", "кто нибудь сможет помочь с",
    "кто-нибудь знает, кто может помочь", "кто нибудь знает кто может помочь", "кто-нибудь знает кто может помочь", "кто нибудь знает, кто может помочь",
    "кто знает кто может помочь", "кто знает, кто может помочь", "кто знает кто сможет помочь", "кто знает, кто сможет помочь", "есть кто сможет помочь", "есть кто может помочь", "есть кто поможет",
    "нужен человек помочь", "нужна помощь с", "ищу человека для помощи", "ищу человека помочь", "ищу кто поможет", "нужен кто поможет",
    "кто поможет с переездом", "кто может помочь с переездом", "кто сможет помочь с переездом", "кто поможет перевезти", "кто может перевезти", "кто сможет перевезти",
}
...
FAKE_PURCHASE_PHRASES = {
    "купите у меня рекламу", "купить у меня рекламу", "закажите у меня рекламу", "купите у меня хоть", "купить у меня хоть", "сделайте заказ для отчета", "сделайте заказ для отчёта",
    "заказ ради отчета", "заказ ради отчёта", "покупка ради отчета", "покупка ради отчёта", "для доказательства покупки", "доказательство покупки", "подтвердить покупку", "подтвердите покупку",
    "доказательство оплаты", "подтвердить оплату", "подтвердите оплату", "скрин оплаты", "скрин покупки", "чек для отчета", "чек для отчёта", "чек для доказательства",
    "я написал что вы купили", "я написал, что вы купили", "я написал что вы у меня купили", "я написал, что вы у меня купили", "написал у себя что вы купили", "написал у себя, что вы купили",
    "можете подтвердить что покупали", "можете подтвердить, что покупали", "можете подтвердить что покупали у меня", "можете подтвердить, что покупали у меня",
}
...
def _has_spaced_number(raw: str, minimum: int = 100) -> bool:
    """Recognize 6000, 6 000, 60 00, 6 00 0, 6 0 0 0 as the same amount."""
    for match in SPACED_DIGITS_RE.finditer(raw):
        digits = re.sub(r"\s+", "", match.group(0))
        if len(digits) >= 3 and int(digits) >= minimum:
            return True
    return False
...
def _is_task_request_spam(normalized: str, spaced: str) -> bool:
    ...
    if any(normalize_spaced(phrase) in spaced or normalize_spaced(phrase).replace(" ", "") in compact for phrase in TASK_REQUEST_PHRASES):
        return True
    ...
    return (request_count >= 1 and action_count >= 1) or (request_count >= 1 and has_task_target)


def _is_paid_task_spam(normalized: str, spaced: str, raw: str) -> bool:
    has_currency_amount = bool(PAID_AMOUNT_RE.search(raw))
    has_spaced_amount = _has_spaced_number(raw)
    ...
    if (has_currency_amount or has_spaced_amount or (has_bare_amount and ("плачу" in raw or "оплата" in raw or has_duration))) and (task_signal_count >= 1 or has_obfuscated_work):
        return True
    if has_people and has_time and has_obfuscated_work and has_bare_amount:
        return True
