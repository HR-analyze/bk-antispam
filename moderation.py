...
SPAM_PHRASES = (
    "заработок от", "заработок в день", "заработок без вложений",
    "заработок на крипте", "легкий заработок", "лёгкий заработок",
    "быстрый заработок", "пассивный доход", "подработка",
    ...
    "требуется персонал", "вакансия", "вакансии", "работа для",
    "ставки на спорт", "букмекер", "казино", "бонус за регистрацию",
    "регистрируйся", "зарегистрируйся", "забери приз", "вы выиграли",
    "вы выиграли приз", "выигрыш", "криптовалюта", "инвестиции с гарантией",
...
PROMO_WORDS = (
    "заработ", "доход", "казино", "ставк", "букмекер", "ваканс",
    "подработ", "знакомств", "эскорт", "интим", "онлифанс", "onlyfans",
)
...
def looks_like_spam(text):
    ...
    if len(normalized) <= 180 and symbols >= 8 and any(phrase in normalized for phrase in CONTACT_PHRASES):
        return True
